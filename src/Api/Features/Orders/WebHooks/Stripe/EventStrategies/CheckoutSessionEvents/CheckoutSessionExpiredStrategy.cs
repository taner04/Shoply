    ...
    ILogger<CheckoutSessionExpiredStrategy> logger) : IStripeEventStrategy
{
    ...
    public async Task HandleNotification(Event stripeEvent, Order order, CancellationToken cancellationToken)
    {
        ...
        LogOrderAndPaymentMarkedAsFailed(logger, order.Id, order.Payment?.Id ?? PaymentId.From(Guid.Empty), stripeEvent.Id);
    }

    [LoggerMessage(LogLevel.Information, "Order {orderId} and Payment {paymentId} marked as failed from Stripe event {eventId} because checkout session expired")]
    static partial void LogOrderAndPaymentMarkedAsFailed(ILogger<CheckoutSessionExpiredStrategy> logger, OrderId orderId, PaymentId paymentId, string eventId);
}