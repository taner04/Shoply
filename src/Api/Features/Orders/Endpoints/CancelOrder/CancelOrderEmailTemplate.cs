...
public sealed class CancelOrderEmailTemplate(Order order, string userEmail) : IEmailTemplate
{
    ...
    private static string BuildHtmlBody(Order order)
    {
        ...
        sb.AppendLine("body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; background-color: #f5f5f5; }");
        sb.AppendLine(".email-container { max-width: 600px; margin: 0 auto; background-color: #ffffff; }");
        sb.AppendLine(".header { background: linear-gradient(135deg, #d35400 0%, #c0392b 100%); color: white; padding: 40px 20px; text-align: center; }");
        ...
        sb.AppendLine(".order-id { background-color: #fff5f5; padding: 15px; border-left: 4px solid #c0392b; margin-bottom: 30px; border-radius: 4px; }");
        sb.AppendLine(".order-id p { font-size: 14px; color: #666; }");
        sb.AppendLine(".order-id .id { font-size: 18px; font-weight: bold; color: #c0392b; }");
        sb.AppendLine("h2 { font-size: 18px; color: #333; margin-bottom: 20px; border-bottom: 2px solid #d35400; padding-bottom: 10px; }");
        sb.AppendLine(".products-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }");
        sb.AppendLine(".products-table th { background-color: #f5f5f5; padding: 12px; text-align: left; font-weight: 600; border-bottom: 2px solid #ddd; }");
        ...
        sb.AppendLine(".summary { background-color: #fff5f5; padding: 20px; border-radius: 4px; margin-bottom: 30px; }");
        sb.AppendLine(".summary-row { display: flex; justify-content: space-between; padding: 8px 0; font-size: 14px; }");
        sb.AppendLine(".summary-row.total { font-size: 18px; font-weight: bold; color: #c0392b; border-top: 2px solid #ddd; padding-top: 12px; }");
        sb.AppendLine(".status-badge { display: inline-block; background-color: #f8d7da; color: #721c24; padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; margin-bottom: 20px; }");
        sb.AppendLine(".info-section { background-color: #fff3cd; padding: 15px; border-left: 4px solid #ffc107; border-radius: 4px; margin-bottom: 20px; font-size: 14px; color: #856404; }");
        sb.AppendLine(".refund-info { background-color: #d4edda; padding: 15px; border-left: 4px solid #28a745; border-radius: 4px; margin-bottom: 20px; font-size: 14px; color: #155724; }");
        sb.AppendLine(".footer { background-color: #f5f5f5; padding: 30px 20px; text-align: center; border-top: 1px solid #ddd; }");
        sb.AppendLine(".footer p { font-size: 12px; color: #999; margin-bottom: 10px; }");
        sb.AppendLine(".cta-button { display: inline-block; background-color: #c0392b; color: white; padding: 12px 30px; text-decoration: none; border-radius: 4px; font-weight: 600; margin: 20px 0; }");
        ...
        sb.AppendLine("<thead><tr><th>Product</th><th>Qty</th><th class=\"price\">Unit Price</th><th class=\"price\">Total</th></tr></thead>");
        sb.AppendLine("<tbody>");
        foreach (var item in order.OrderItems)
        {
            sb.AppendLine($"<tr><td><strong>{item.ProductName}</strong><br><small>{item.ProductDescription}</small></td><td>{item.Quantity}</td><td class=\"price\">{item.UnitPrice:C}</td><td class=\"price\">{item.TotalPrice:C}</td></tr>");
        }
        sb.AppendLine("</tbody>");
        sb.AppendLine("</table>");
        ...
        sb.AppendLine("<p><a href=\"#\" style=\"color: #c0392b; text-decoration: none;\">Unsubscribe</a> | <a href=\"#\" style=\"color: #c0392b; text-decoration: none;\">Privacy Policy</a></p>");
        sb.AppendLine("</div>");
