    ...
    ILogger<CheckoutSessionPaymentSucceededStrategy> logger) : IStripeEventStrategy
{
    ...
    public async Task HandleNotification(Event stripeEvent, Order order, CancellationToken cancellationToken)
    {
        ...
        LogOrderAndPaymentMarkedAsSucceeded(logger, order.Id, order.Payment?.Id ?? PaymentId.From(Guid.Empty), stripeEvent.Id);
    }

    [LoggerMessage(LogLevel.Information, "Order {orderId} and Payment {paymentId} marked as succeeded from Stripe event {eventId}")]
    static partial void LogOrderAndPaymentMarkedAsSucceeded(ILogger<CheckoutSessionPaymentSucceededStrategy> logger, OrderId orderId, PaymentId paymentId, string eventId);
}