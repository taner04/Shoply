    ...
    ILogger<RefundUpdatedStrategy> logger) : IStripeEventStrategy
{
    ...
    [LoggerMessage(LogLevel.Information, "Refund succeeded for order {orderId} payment {paymentId}, amount: {amount} from Stripe event {eventId}")]
    static partial void LogRefundSucceeded(ILogger<RefundUpdatedStrategy> logger, OrderId orderId, PaymentId paymentId, decimal amount, string eventId);

    [LoggerMessage(LogLevel.Warning, "Refund failed for order {orderId} payment {paymentId} from Stripe event {eventId}")]
    static partial void LogRefundFailed(ILogger<RefundUpdatedStrategy> logger, OrderId orderId, PaymentId paymentId, string eventId);
    ...
    static partial void LogRefundPaymentIntentMismatch(ILogger<RefundUpdatedStrategy> logger, string refundId, string paymentIntentId, string eventId);
}