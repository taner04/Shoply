    ...
    ILogger<CheckoutSessionPaymentFailedStrategy> logger) : IStripeEventStrategy
{
    ...
    public async Task HandleNotification(Event stripeEvent, Order order, CancellationToken cancellationToken)
    {
        ...
        LogOrderAndPaymentMarkedAsFailed(logger, order.Id, order.Payment?.Id ?? PaymentId.From(Guid.Empty), "Payment failed", stripeEvent.Id);
    }

    [LoggerMessage(LogLevel.Information, "Order {orderId} and Payment {paymentId} marked as failed from Stripe event {eventId}: {failureReason}")]
    static partial void LogOrderAndPaymentMarkedAsFailed(ILogger<CheckoutSessionPaymentFailedStrategy> logger, OrderId orderId, PaymentId paymentId, string failureReason, string eventId);
}